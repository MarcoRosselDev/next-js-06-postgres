...
echo "$($PSQL "SELECT DISTINCT(course) FROM majors FULL JOIN students USING(major_id) FULL JOIN majors_courses USING(major_id) FULL JOIN courses USING(course_id) WHERE student_id IS NULL OR first_name='Obie' ORDER BY course DESC")"
